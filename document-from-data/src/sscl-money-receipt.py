#!/usr/bin/env python3
'''
    generate templated money receipt from data
'''
...
def post_process_data(processed_data):
    for item in processed_data['data']:
        item['totalinwords'] = num2words.num2words(item['amount'].replace(',', ''), to='currency', lang='en_IN').replace('euro', 'taka').replace('cents', 'paisa')
